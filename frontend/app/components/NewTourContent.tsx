...
// The "add a tour" form, shared between the standalone /dashboard/new-tour
// page (direct link/bookmark) and NewTourModal (opened as a popup from the
// "+ Tur əlavə et" button inside the operator panel page). onSuccess fires
// after the tour is published so the caller can navigate away (standalone
// page) or just close the popup and refresh the panel's tour list (modal).
export default function NewTourContent({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <div>
      <h1 className="font-display text-xl font-bold text-foreground mb-1 pr-10">{t('tourForm.addTitle')}</h1>
