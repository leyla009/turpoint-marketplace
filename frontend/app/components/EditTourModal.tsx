...
import NewTourContent, { type ExistingTour } from './NewTourContent';

// The Pencil icon's edit action, opened as a popup right on the panel page
// instead of navigating to /dashboard/edit-tour/:id - same pattern as
// NewTourModal, reusing the identical form (pre-filled) via NewTourContent.
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] bg-black/60 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <NewTourContent tour={tour} onSuccess={onUpdated} />
      </div>
    </div>,
    document.body
  );
